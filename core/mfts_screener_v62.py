...
DEBUG_V62 = os.environ.get("MFTS_V62_DEBUG", "false").lower() == "true"


def _v62_debug(msg: str) -> None:
    if DEBUG_V62:
        print(f"[v6.2][debug] {msg}")
...
def calculate_alpha_score_v62(row, thresholds):
    ...
    try:
        ...
        _v62_debug(f"风险扣分(流动性陷阱)计算异常: {e}")
    ...
    try:
        ...
        _v62_debug(f"Volume Ratio 因子计算异常: {e}")
    ...
    try:
        ...
        _v62_debug(f"52W Position 因子计算异常: {e}")
    ...
    try:
        ...
        _v62_debug(f"Momentum 因子计算异常: {e}")
    ...
    try:
        ...
        _v62_debug(f"Wyckoff Spring 计算异常: {e}")
...
def scan_v62(df, target_date=None, meta_dict=None, top_n=30):
    ...
    # 添加股票名称
    def get_meta_name(code):
        val = meta_dict.get(code, '')
        if isinstance(val, dict):
            return val.get('name', code)
        return val if val else code

    result['name'] = result['ts_code'].apply(get_meta_name)
